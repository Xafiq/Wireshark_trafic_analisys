...
*These instructions are [disponibles en español](https://github.com/breatheco-de/set-up-an-SSL-in-openSSL-with-a-secure-server/blob/main/README.md)*
...
- Open a web browser on Debian Linux.
- Navigate to the DVWA login page (e.g., http://[IP-OF-METASPLOITABLE]/dvwa/login.php).
...
- In the filter box at the bottom of the Wireshark window, enter `host [IP-OF-DEBIAN] and http` and press Enter. This will capture all HTTP requests from the Debian machine.

**Step 3: Perform an XSS attack in DVWA**

- Open a web browser on Kali Linux.
- Navigate to the DVWA site (e.g., http://[IP-OF-METASPLOITABLE]/dvwa/vulnerabilities/xss_r.php).
...
## 5 - Capture and analyze a request to cocacola
...
**Paso 2: Apply filter TCP for HTTP traffic**

- In the filter box at the bottom of the Wireshark window, enter `tcp.port == 80` and press Enter. This will capture all HTTP traffic.

**Paso 3: Make an HTTP request in the browser**
